...
- `.gitattributes` —  (~5 tok)
- `.gitignore` —  (~14 tok)
- `PLAN.md` — zrouter 设计与实施计划 (~4624 tok)
- `README.md` — zrouter (~939 tok)
- `build.zig` — pub fn build (~457 tok)
- `build.zig.zon` — Zig package manifest (~1140 tok)
- `install.ps1` —  (~1510 tok)
- `install.sh` —  (~1228 tok)
- `skill/`
  - `SKILL.md` — zrouter (~2557 tok)
  - `configuration.md` — zrouter configuration (~1182 tok)
  - `templates.md` — zrouter templates (~904 tok)
- `src/`
  - `assets/default.toml` — zrouter built-in defaults — embedded at compile time. (~908 tok)
  - `assets/filters.toml` — zrouter built-in command output filters — embedded at compile time. (~71 tok)
  ...
  - `main.zig` — pub fn main (~6229 tok)
  - `pipe.zig` — pub fn run (~750 tok)
  - `root.zig` —  (~49 tok)
